...
class Root(rend.Page):
    ...
    def _fmt_time_delta(self, time_1, time_2=None):
        if time_1 is None:
    ...
    def render_service_row(self, ctx, server):
        ...
        if rhost:
            ...
            service_connection_status_abs_time, service_connection_status_rel_time = self._fmt_time_delta(server.get_last_connect_time())
        else:
            addr = "N/A"
            service_connection_status = "Disconnected"
            service_connection_status_abs_time, service_connection_status_rel_time = self._fmt_time_delta(server.get_last_loss_time())

        last_received_data_abs_time, last_received_data_rel_time = self._fmt_time_delta(server.get_last_received_data_time())
